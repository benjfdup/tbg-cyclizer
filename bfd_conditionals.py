...
class w_t(): # make this an abstract class
    ...
    Instances should be created with one of the available class methods, not
    with the __init__ method directly (which will result in a warning)
    '''

    def __init__(self, w_t: Callable[[float], float], loss_type: str, warn_user=True):
        '''
        Not to be directly used. Creates a w_t object to handle conditional loss func coefficients
        as a function of inference time (which is ∈ (0, 1]).
        '''

        if warn_user:
            warnings.warn(
                "Direct instantiation of w_t is discouraged. Use the provided class methods instead.",
                stacklevel=1,
            )
        self.w_t = w_t
    ...
    def __call__(self, t: float):
        assert 0.0 <= t <= 1.0, "t must be between 0.0 and 1.0"
        
        out = self.w_t(t)
        if not 0.0 <= out <= 1.0:
            warnings.warn(
                f'w_t functions are not supposed to return values out of (0, 1]. This call is yielding a value of {out}',
                stacklevel=1
            )
    ...
    @classmethod
    def gaussian_w_t(cls, mu, s, coeff = 1):
        '''
        ...
        w_t.__doc__ = f'''
        Returns the loss coefficient for the specified moment in time. This loss is drawn from the
        "pseudo-gaussian" "family" with params mu: {mu}, s: {s}, coeff: {coeff}.
    ...
    @classmethod
    def maxwell_boltzmann_w_t(cls, a):
        '''
        ...
        w_t.__doc__ = f'''
        returns the loss coefficient for the specified moment in time. This loss is a maxwell-boltzman
        fn w/ a = {a}
        '''
    ...
    @classmethod
    def constant_w_t(cls, c):
        '''
        returns a function that is a constant c wrt time.
        '''

        if c > 1.0:
            warnings.warn('This function will output values out ov (0, 1]', 
                          stacklevel= 1,
                          )

        w_t = lambda t: torch.tensor(c)

        w_t.__doc__ = f"""
         returns the loss coefficient for the specified moment in time. This loss fn is just a constant
         c = {c}
        """
